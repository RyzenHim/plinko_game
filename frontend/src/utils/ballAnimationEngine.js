...
/** Minimum center-to-center distance — ball never overlaps peg */
...
/** Ball center offset so surface clears peg along approach ray */
...
/** Build timeline with explicit bin index */
export function buildBallTimelineFull(path, dropColumn, binIndex, { reducedMotion = false } = {}) {
  ...
  if (path.length > 0) {
    ...
    pushSegment(
      segments,
      { ...pos, sx: 1, sy: 1, rot: 0 },
      { ...firstContact, sy: 1.02, sx: 0.98, rot: (firstContact.x - pos.x) * 0.08 },
      0.20 * speed,
      EASE.fall,
      { type: "fall" }
    );
    pos = state(firstContact.x, firstContact.y, 0.98, 1.02, (firstContact.x - pos.x) * 0.08);
  }
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    // Compression segment at contact (polished steel ball squashes very slightly)
    const compressedState = { ...contact, sx: 1.03, sy: 0.97, rot: exitDir * 4 };
    pushSegment(
      segments,
      ...
      0.040 * speed,
      EASE.impact,
      { type: "impact", pegId: `peg-${step.row}-${step.pegIndex}`, spark: true, sound: "tick" }
    );
    ...
    if (nextStep) {
      ...
      // Define cubic Bezier control points for the flight trajectory
      const p0 = contact;
      const p1 = { x: contact.x + exitDir * 10, y: contact.y - 12 }; // Upward/outward bounce energy
      const p2 = { x: nextContact.x - exitDir * 2, y: nextContact.y - 16 }; // Gravity acceleration pull
      ...
      pushSegment(
        ...
        0.28 * speed,
        (t) => t, // Linear parameter sweep since curve shape itself controls velocity
        { 
          type: "arc",
          isBezier: true,
          p0, p1, p2, p3
      ...
      // Last peg collision: trajectory down to the bin slot approach point
      const binPos = getBinPosition(binIndex);
      const floorY = binPos.y + 2;
      const landApproach = { x: binPos.x, y: floorY - BALL_RADIUS - 2, sx: 1, sy: 1, rot: 0 };
  ...
  pushSegment(segments, { ...pos }, landImpact, 0.05 * speed, EASE.impact, { type: "land_impact" });

  // Bounce 1 (stretch upwards)
  const bounce1 = { x: binPos.x, y: floorY - 11, sx: 0.96, sy: 1.04, rot: 0 };
  pushSegment(segments, landImpact, bounce1, 0.08 * speed, EASE.rebound, { type: "land_bounce1" });

  // Re-impact 2 (minor compression)
  const landImpact2 = { x: binPos.x, y: floorY, sx: 1.02, sy: 0.98, rot: 0 };
  pushSegment(segments, bounce1, landImpact2, 0.07 * speed, EASE.fall, { type: "land_impact2" });

  // Bounce 2 (small rebound)
  const bounce2 = { x: binPos.x, y: floorY - 4, sx: 0.99, sy: 1.01, rot: 0 };
  pushSegment(segments, landImpact2, bounce2, 0.06 * speed, EASE.rebound, { type: "land_bounce2" });

  // Settle back to slot floor
  const settle = { x: binPos.x, y: floorY + 2, sx: 1, sy: 1, rot: 0 };
  pushSegment(segments, bounce2, settle, 0.10 * speed, EASE.settle, { type: "land_settle" });
...
/** Sample transform at elapsed seconds */
export function sampleTimeline(segments, elapsed) {
  ...
  for (const seg of segments) {
    ...
    if (elapsed <= end) {
      ...
      if (seg.isBezier) {
        ...
        const ptCurr = cubicBezier(seg.p0, seg.p1, seg.p2, seg.p3, t);
        const frameDist = Math.hypot(ptCurr.x - ptPrev.x, ptCurr.y - ptPrev.y);
        velocity = (frameDist / (0.02 * seg.duration)) * 0.15;
      } else {
        velocity = seg.duration > 0 ? (dist / seg.duration) * (1 - Math.abs(0.5 - t) * 0.5) : 0;
      }
  ...
  const last = segments[segments.length - 1]?.to ?? { x: 0, y: 0, sx: 1, sy: 1, rot: 0 };
  return { ...last, sx: last.sx ?? 1, sy: last.sy ?? 1, rot: last.rot ?? 0, segment: null, progress: 1, velocity: 0 };
}
